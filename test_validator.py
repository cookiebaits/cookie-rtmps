...
class TestStreamValidator(unittest.TestCase):
    ...
    def test_is_ip_allowed_loopback(self):
        self.assertTrue(stream_validator.is_ip_allowed("127.0.0.1", "203.0.113.1"))
        self.assertTrue(stream_validator.is_ip_allowed("::1", "203.0.113.1"))

    def test_is_ip_allowed_empty_setting(self):
        self.assertTrue(stream_validator.is_ip_allowed("203.0.113.45", ""))
        self.assertTrue(stream_validator.is_ip_allowed("198.51.100.100", None))

    def test_is_ip_allowed_matching_ip(self):
        whitelist = "203.0.113.10, 198.51.100.50"
        self.assertTrue(stream_validator.is_ip_allowed("203.0.113.10", whitelist))
        self.assertTrue(stream_validator.is_ip_allowed("198.51.100.50", whitelist))
    ...
    def test_is_ip_allowed_non_matching_ip(self):
        ...
        self.assertFalse(stream_validator.is_ip_allowed("198.51.100.2", whitelist))

    def test_is_ip_allowed_with_port(self):
        whitelist = "203.0.113.10"
        self.assertTrue(stream_validator.is_ip_allowed("203.0.113.10:54321", whitelist))
    ...
    def test_is_ip_allowed_cidr(self):
        ...
        self.assertTrue(stream_validator.is_ip_allowed("198.51.100.5", whitelist))
        self.assertFalse(stream_validator.is_ip_allowed("203.0.113.5", whitelist))

    @patch.object(stream_validator, 'VALID_KEYS', ['test_key_123'])
    @patch.object(stream_validator, 'ACCEPTED_IP', '203.0.113.10')
    def test_validate_route_whitelisting(self):
        client = stream_validator.app.test_client()

        # Whitelisted IP -> 200 OK
        resp = client.post('/validate', data={'name': 'test_key_123', 'addr': '203.0.113.10', 'app': 'live'})
        self.assertEqual(resp.status_code, 200)

        # Non-whitelisted public IP -> 403 Forbidden
